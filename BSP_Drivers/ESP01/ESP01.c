...
void SendATCmd(char *cmd, int waitms)
{
	if (NULL != cmd)
	{
		HAL_UART_Transmit_DMA(pESPHandle, (uint8_t *)cmd, strlen(cmd));
		osDelay(waitms);
	}
}
...
{
	if (NULL != str)
		HAL_UART_Transmit_DMA(pESPHandle, (uint8_t *)str, strlen(str));
}
...
void InitEsp01(UART_HandleTypeDef* pUartHandle)
{
	...
	HAL_UART_Receive_DMA(pESPHandle, g_esp01.rxdata.rx_buf, sizeof(g_esp01.rxdata.rx_buf) - 2);
	HAL_UART_AbortReceive_IT(pESPHandle);
	
	HAL_UARTEx_ReceiveToIdle_DMA(pESPHandle, g_esp01.rxdata.rx_buf, sizeof(g_esp01.rxdata.rx_buf) - 2);
...
void EspRxEvent(UART_HandleTypeDef *huart, uint16_t Size)
{
	if (huart == pESPHandle)
	{
		if (Size > 0)
...
// 串口接收事件回调函数，如果要支持多个串口接收，把该函数移到FreeRTOS.c文件末尾，并在ESP01.h中将RxEvent函数声明一遍
void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef *huart, uint16_t Size)
{
	EspRxEvent(huart, Size);
}
